...
import { dictTagFun } from "@/utils/dict";
...
export default {
  print: (params, printFrame) => {
    ...
    params.properties = params.properties.map((property) => {
      return {
        ...property,
        // dictOptions: typeof property === "object" ? property.dictOptions : property,
    ...
    for (let n = 0; n < properties.length; n++) {
      ...
      if (property.length > 1) {
        for (let p = 0; p < property.length; p++) {
          if (!!properties[n].dictOptions) {
            stringData = dictTagFun(properties[n].dictOptions, stringData[property[p]]);
          } else{
        ...
        if (!!properties[n].dictOptions) {
          stringData = dictTagFun(properties[n].dictOptions, stringData[properties[n].field]);
        } else{
